...
- On 2026-09-18, the repository was on `main`; the expense/private-evidence/staff-lookup upgrade was verified locally and remained uncommitted at the end of that implementation turn.
- `npm run typecheck`, `npm test` (166 tests), and the Webpack production build (64 generated pages) passed after the register-shift, exception-review, multi-location transfer, batch/expiry, freshness-forecast, inventory-disposal, MongoDB Atlas M0/Stable API compatibility, serverless cold-start performance, customer-account credit controls, quotation-to-invoice, quote-linked delivery-order, controlled CSV bank-reconciliation, transactional month-end-close, fixed-asset/depreciation/disposal, and employee-expense/private-evidence upgrades. On this Windows host, the successful build used `KONKON_DISABLE_WEBPACK_CACHE=1` after Webpack's optional pack cache hit a filesystem write error.
