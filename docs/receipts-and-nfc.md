...
Authenticated staff can now use **E-invoice files** on the receipt/invoice screen to generate general UBL XML, accounting JSON or limited domestic MyInvois preparation, with encrypted immutable history. See [country documents and remote NFC](country-documents.md). The public receipt endpoint below remains a privacy-minimized preparation object and does not expose these private buyer/supplier files.
...
4. To use a card you already have, choose **Connect another phone** on the member's card page, scan its private QR with a compatible NFC phone, then hold the card near the phone. The pass automatically starts listening; a first-use browser permission may require one tap on **Start NFC (if needed)**. Confirm the target member on the counter. The pass is member-locked and cannot be retargeted by POS/inventory. Same-phone reading remains available. The system does not write to the card: it hashes the browser-provided serial number when available, otherwise a digest of the readable NDEF message, and stores only a keyed fingerprint under that member. A bound card can then be scanned from POS, Members or a linked lookup phone pass to select that member.
5. Use **Link a phone** on POS or Members, open the scanner pass on that phone, and tap an issued or previously bound card. Barcode scanning and NFC are separate lanes; the desktop receives only the opaque membership code and performs an authenticated active-card/member lookup. POS selects the member; Members shows the matching profile.
6. **Suspend** is reversible; **Void** is permanent. **Delete** retires the binding while preserving card, membership and accounting audit history. A replacement issued card receives a new random credential. Existing legacy Code 39 member cards are managed separately; retiring one NFC binding does not revoke a different card.
7. If a member is archived or removed, their existing-card binding appears under **Members → Orphaned NFC registrations**. This list contains only `BOUND` cards whose member is no longer active (including a binding that was already marked `VOID`) and never exposes the raw NFC fingerprint. Staff with `members.write` can clear a row after confirmation. POS can do the same without opening Members: scan the bound `KKNT1` code with the USB/Bluetooth/linked-phone scanner, review the archived-member notice, then choose **Clear NFC registration**. The API checks the card kind and archived/missing member again, so an active member's card cannot be cleared through this action.

NFC requires HTTPS (localhost is allowed for development), a compatible NFC-enabled Android phone, Chrome and a readable tag that Chrome exposes. Safari/iPhone and many desktop browsers do not expose Web NFC; use QR or a keyboard-wedge reader there. Web NFC is limited to NDEF messages, although Chrome may also provide the tag's `serialNumber`; a hotel or TNG card that exposes neither cannot be identified by a web page and needs a native app or supported USB/NFC reader. NFC permissions require a user gesture. Writing replaces the tag's current NDEF content, so use a dedicated membership tag. The writer does not lock a tag read-only.
...
- Existing-card bindings store a server-keyed HMAC of a client-derived digest (`KKNT1-S` for a browser-provided serial, `KKNT1-N` for an NDEF digest), never the raw serial or tag contents. NDEF-only bindings can collide when two cards contain identical public data; prefer serial-backed bindings or dedicated issued cards where uniqueness matters. A copied NDEF message can reproduce an NDEF binding, so require staff/customer verification before high-value rewards.
