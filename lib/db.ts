...
async function initializeIndexes(db: Db) {
  ...
  await Promise.all([
    ...
    db.collection("inventoryBalances").createIndex({ productId: 1, locationId: 1 }, { unique: true }),
    db.collection("inventoryBalances").createIndex({ locationId: 1, quantity: 1, productName: 1 }),
    db.collection("inventoryActivations").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("inventoryActivations").createIndex({ productId: 1 }, { unique: true }),
    db.collection("inventoryBatches").createIndex({ productId: 1, locationId: 1, lotKey: 1, expiryDate: 1 }, { unique: true }),
    db.collection("inventoryBatches").createIndex({ locationId: 1, expiryDate: 1, quantity: 1 }),
    db.collection("inventoryBatches").createIndex({ productId: 1, expiryDate: 1, quantity: 1 }),
    db.collection("inventoryBatches").createIndex({ lotKey: 1, createdAt: 1 }),
    db.collection("inventoryBatchActivations").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("inventoryBatchActivations").createIndex({ productId: 1 }, { unique: true }),
    db.collection("inventoryBatchEvents").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("inventoryBatchEvents").createIndex({ batchId: 1, createdAt: -1 }),
    db.collection("inventoryBatchEvents").createIndex({ productId: 1, lotKey: 1, createdAt: -1 }),
    db.collection("stockTransfers").createIndex({ transferNo: 1 }, { unique: true }),
    db.collection("stockTransfers").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("stockTransfers").createIndex({ status: 1, dispatchedAt: -1 }),
    db.collection("stockTransfers").createIndex({ sourceLocationId: 1, destinationLocationId: 1, status: 1 }),
    db.collection("stockTransfers").createIndex({ "items.batchAllocations.lotKey": 1, dispatchedAt: -1 }),
    ...
    db.collection("sales").createIndex({ "items.batchAllocations.lotKey": 1, createdAt: -1 }),
    db.collection("sales").createIndex(
    ...
    db.collection("registerShifts").createIndex({ openRequestId: 1 }, { unique: true }),
    db.collection("registerShifts").createIndex(
      { closeRequestId: 1 },
      { unique: true, partialFilterExpression: { closeRequestId: { $type: "string" } } },
    ),
    db.collection("registerShifts").createIndex(
    ...
    db.collection("deliveryOrders").createIndex({ deliveryOrderNo: 1 }, { unique: true }),
    db.collection("deliveryOrders").createIndex(
      { sourceQuoteId: 1 },
      { unique: true, partialFilterExpression: { sourceQuoteId: { $type: "objectId" } } },
    ),
    ensureStableOptionalStringUniqueIndex(db, "deliveryOrders", "clientRequestId"),
    db.collection("deliveryOrders").createIndex({ status: 1, scheduledDate: 1, createdAt: -1 }),
    ...
    db.collection("operationalReviews").createIndex({ reviewNo: 1 }, { unique: true }),
    db.collection("operationalReviews").createIndex({ sourceType: 1, sourceId: 1, ruleCode: 1 }, { unique: true }),
    db.collection("operationalReviews").createIndex({ status: 1, severityRank: -1, occurredAt: -1 }),
    db.collection("operationalReviews").createIndex({ category: 1, status: 1, occurredAt: -1 }),
    ...
    db.collection("localPaymentEvents").createIndex({ eventId: 1 }, { unique: true }),
    db.collection("localPaymentEvents").createIndex({ provider: 1, currency: 1, paidAt: -1 }),
    db.collection("localPaymentEvents").createIndex({ status: 1, createdAt: -1 }),
    ...
    db.collection("paymentDisplaySessions").createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }),
    db.collection("paymentDisplaySessions").createIndex({ createdBy: 1, expiresAt: -1 }),
    ...
    db.collection("purchaseOrders").createIndex({ purchaseOrderNo: 1 }, { unique: true }),
    db.collection("purchaseOrders").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("purchaseOrders").createIndex({ supplierId: 1, status: 1, expectedDate: 1 }),
    db.collection("purchaseOrders").createIndex({ locationId: 1, status: 1, createdAt: -1 }),
    db.collection("goodsReceipts").createIndex({ receiptNo: 1 }, { unique: true }),
    db.collection("goodsReceipts").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("goodsReceipts").createIndex({ purchaseOrderId: 1, createdAt: -1 }),
    db.collection("goodsReceipts").createIndex({ "items.lotKey": 1, receivedAt: -1 }),
    db.collection("accountsPayableBills").createIndex({ billNo: 1 }, { unique: true }),
    db.collection("accountsPayableBills").createIndex({ supplierId: 1, supplierInvoiceNoNormalized: 1 }, { unique: true }),
    db.collection("accountsPayableBills").createIndex({ status: 1, dueDate: 1 }),
    db.collection("supplierPayments").createIndex({ paymentNo: 1 }, { unique: true }),
    db.collection("supplierPayments").createIndex({ clientRequestId: 1 }, { unique: true }),
    db.collection("supplierPayments").createIndex(
      { paymentAccountCode: 1, referenceNormalized: 1 },
      { unique: true, partialFilterExpression: { referenceNormalized: { $type: "string" } } },
    ),
