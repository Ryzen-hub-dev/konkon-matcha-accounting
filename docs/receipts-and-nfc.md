...
4. To use a card you already have, open the member's card page on a supported NFC phone and press **Bind an existing NFC card**. Tap the hotel, transit or other tag. The system does not write to it: it hashes the browser-provided serial number when available, otherwise a digest of the readable NDEF message, and stores only a keyed fingerprint under that member. A bound card can then be scanned from POS, Members or a linked phone pass to select that member.
