...
- On 2026-09-20, deployed `main` reached `4c264e9` with controlled annual budgets, cost-centre/project and percentage-allocation accounting, invoice/product dimension defaults, deterministic POS refund inheritance, audited purchase-order short-close, AP aging/supplier exposure/CSV export, a searchable printable bilingual Help centre and complete Chinese/English user manuals. The verified Atlas M0 v8 index baseline remains unchanged and concurrency-sensitive additions use transactional business-key locks.
- The current working tree adds controlled purchase requisitions, maker-checker review and one-time transactional purchase-order conversion. It passes `npm run typecheck`, all 197 tests and the Webpack production build with 68 generated pages/routes. On this Windows host, use `KONKON_DISABLE_WEBPACK_CACHE=1` if Webpack's optional pack cache encounters a filesystem write error.
