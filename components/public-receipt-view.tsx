...
export function PublicReceiptView({ branding }: { branding: PublicBranding }) {
  ...
  const receiptBrand = receipt?.businessSnapshot?.businessName || branding.businessName;
  const receiptLogo = receipt?.businessSnapshot?.workspaceLogoDataUrl || branding.workspaceLogoDataUrl;
  return <main className="public-receipt-page"><header className="no-print"><span className="public-brand-line">{receiptLogo ? <img src={receiptLogo} alt={`${receiptBrand} logo`} /> : null}<b>{receiptBrand}</b></span><span className="eyebrow">DIGITAL RECEIPT</span><h1>Your receipt</h1><p>Keep a copy for your records. For a return, show this receipt to the store.</p></header>
