...
import { Award, BadgeCheck, CreditCard, Fingerprint, Mail, Nfc, Pencil, Phone, Search, ShieldCheck, Trash2, UserPlus, Users } from "lucide-react";
...
export function MembersView({ canWrite = false }: { canWrite?: boolean }) {
  ...
  async function archive(member: MemberRecord) { if (!window.confirm(`Delete ${member.name}? Their contact and identity lookup details will be removed and issued cards revoked. Phone/identity details can be registered again. Sales and reward history stay intact; existing NFC cards remain available for safe orphan-card cleanup.`)) return; try { await apiRequest("/api/members", { method: "DELETE", body: JSON.stringify({ id: member._id }) }); signalMemberChange(); show("Member deleted; contact details released and financial history preserved."); await load(); } catch (reason) { show(reason instanceof Error ? reason.message : "Could not delete the member.", "error"); } }
