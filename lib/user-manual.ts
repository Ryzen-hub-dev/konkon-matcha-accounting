...
export const USER_MANUAL_TOPICS: ManualTopic[] = [
  ...
  },
  {
    ...
    zh: { title: "供应商、采购与智能补货", summary: "从内部采购申请或补货建议建立承诺，审批后分批收货并自动生成应付。", steps: ["在 Purchasing & payables 建立供应商、币种、账期、交期和最低订单。", "需要内部批准时先建立采购申请，填写地点、需求日期、理由、商品与数量，再由不同授权人员审批。", "已批准申请可一键带入采购单；系统锁定已批准地点、商品和数量，同时允许选择供应商及确认成本。", "查看 Replenishment queue；建议结合库存、在途量、补货点和近 30 日需求。", "建立采购单草稿，选择供应商、地点、日期、税和商品成本。", "由授权人员审批；非 Owner 通常不能审批自己建立的采购单。", "实际到货时仅输入本次收到数量、供应商发票号、批次和日期。", "最后不会再到货时，对部分收货单使用 Close remainder 并填写原因。"], tips: ["采购申请转换和收货都保留不可替换的来源与操作者证据。", "收货会在一个数据库交易中更新库存、加权成本、应付、总账和审计。", "智能补货是建议，订货前仍须确认季节性、促销和供应商能力。"] },
    en: { title: "Suppliers, purchasing and smart replenishment", summary: "Create commitments from internal requisitions or replenishment advice, approve them and receive in stages into payables.", steps: ["In Purchasing & payables, create a supplier with currency, terms, lead time and minimum order.", "When internal approval is required, submit a requisition with location, required date, justification, products and quantities for review by a different authorised user.", "Convert an approved requisition into a purchase order; the approved destination, products and quantities stay locked while supplier and cost are confirmed.", "Review the Replenishment queue; advice combines stock, inbound quantity, reorder level and recent 30-day demand.", "Create a purchase-order draft with supplier, destination, date, tax and product costs.", "Have an authorised user approve it; non-Owners normally cannot approve their own order.", "On physical delivery, enter only the quantity received now, supplier invoice number, batches and dates.", "If no more goods will arrive, use Close remainder on a partially received order and record the reason."], tips: ["Requisition conversion and goods receipt both retain permanent source and operator evidence.", "Receipt updates inventory, weighted cost, payable, general ledger and audit together in one database transaction.", "Smart replenishment is advice; check seasonality, promotions and supplier capacity before ordering."] },
  },
  {
