...
type DashboardData = {
  ...
  recentSales: Array<{ _id: string; receiptNo: string; memberName: string; total: number; refundedAmount?: number; paymentMethod: string; paymentMethodName?: string; status?: string; createdAt: string }>;
...
function SalesChart({ points }: { points: DashboardData["dailySales"] }) {
  ...
  return <div className="chart-wrap">
    ...
    <div className="chart-labels">{coords.map((point) => <span key={point._id}>{new Date(`${point._id}T00:00:00Z`).toLocaleDateString(profile.locale, { weekday: "short", timeZone: "UTC" })}</span>)}</div>
  </div>;
}
...
export function DashboardView() {
  ...
  const greeting = useMemo(() => { const hour = Number(new Intl.DateTimeFormat("en", { hour: "numeric", hourCycle: "h23", timeZone: profile.timeZone }).format(new Date())); return hour < 12 ? "Ohayō" : hour < 18 ? "Good afternoon" : "Konbanwa"; }, [profile.timeZone]);
  ...
  return <div className="page page-enter">
    ...
    <section className="dashboard-grid">
      <article className="panel sales-panel"><header className="panel-header"><div><span className="eyebrow">SALES FLOW</span><h2>Seven-day rhythm</h2></div><span className="panel-note">{profile.currency} · DAILY</span></header><SalesChart points={value.dailySales} /></article>
      <article className="panel"><header className="panel-header"><div><span className="eyebrow">FRESHLY SERVED</span><h2>Recent receipts</h2></div><Link href="/receipts">View all</Link></header>
        {value.recentSales.length ? <div className="receipt-list">{value.recentSales.map((sale) => <Link href={`/receipts/${sale._id}`} className="receipt-row" key={sale._id}><div className="receipt-icon"><ReceiptText size={17} /></div><div><strong>{sale.memberName}</strong><span>{sale.receiptNo} · {dateTime.format(new Date(sale.createdAt))}</span></div><div><strong>{money.format(Math.max(0, sale.total - Number(sale.refundedAmount || 0)))}</strong><span>{sale.status === "COMPLETED" || !sale.status ? sale.paymentMethodName || sale.paymentMethod : sale.status.replaceAll("_", " ")}</span></div></Link>)}</div> : <EmptyState title="No cups rung up yet" detail="Complete the first sale from the POS and it will appear here." />}
      </article>
