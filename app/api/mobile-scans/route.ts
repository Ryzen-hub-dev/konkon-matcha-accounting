...
import { scannerPurposeFilter } from "@/lib/scanner-routing";
...
async function claimPendingEvents(db: Awaited<ReturnType<typeof getDb>>, scannerSessionId: ObjectId, consumerId: string, purpose: "POS" | "INVENTORY") {
...
async function waitForPendingEvents(db: Awaited<ReturnType<typeof getDb>>, scannerSessionId: ObjectId, consumerId: string, purpose: "POS" | "INVENTORY", wait: boolean) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const purpose = session.purpose === "INVENTORY" ? "INVENTORY" : "POS";
    const event = { scannerSessionId: session._id, purpose, code, consumedAt: null, expiresAt: session.expiresAt, createdAt: now };
