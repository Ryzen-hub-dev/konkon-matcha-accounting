...
export const USER_MANUAL_TOPICS: ManualTopic[] = [
  ...
  },
  {
    ...
    zh: { title: "供应商、询价、采购与智能补货", summary: "从采购申请或补货建议建立承诺，对比供应商报价后分批收货并自动生成应付。", steps: ["在 Purchasing & payables 建立供应商、币种、账期、交期和最低订单。", "需要内部批准时先建立采购申请，填写地点、需求日期、理由、商品与数量，再由不同授权人员审批。", "需要比价时从已批准申请建立 RFQ，邀请至少两家供应商并记录每家报价和承诺交期。", "在 RFQ comparison 查看冻结的本位币比较金额；由不同授权人员选择中标报价并填写理由。", "将中标报价转为采购单；供应商、价格、交期、地点、商品和数量不能被替换。", "查看 Replenishment queue；建议结合库存、在途量、补货点和近 30 日需求。", "由授权人员审批采购单；非 Owner 通常不能审批自己建立的采购单。", "实际到货时仅输入本次收到数量、供应商发票号、批次和日期。", "最后不会再到货时，对部分收货单使用 Close remainder 并填写原因。"], tips: ["系统只标出最低比较价，不会自动选择供应商。", "询价和采购申请本身都不会移动库存或过账会计。", "收货会在一个数据库交易中更新库存、加权成本、应付、总账和审计。"] },
    en: { title: "Suppliers, RFQs, purchasing and smart replenishment", summary: "Create commitments from requisitions or replenishment advice, compare supplier offers, then receive in stages into payables.", steps: ["In Purchasing & payables, create a supplier with currency, terms, lead time and minimum order.", "When internal approval is required, submit a requisition with location, required date, justification, products and quantities for review by a different authorised user.", "When competition is useful, open an RFQ from the approved requisition, invite at least two suppliers and record each offer and promised date.", "Review frozen base-currency comparisons in RFQ comparison; a different authorised user awards an offer with a reason.", "Convert the award into a purchase order; supplier, price, delivery date, destination, products and quantities cannot be substituted.", "Review the Replenishment queue; advice combines stock, inbound quantity, reorder level and recent 30-day demand.", "Have an authorised user approve the purchase order; non-Owners normally cannot approve their own order.", "On physical delivery, enter only the quantity received now, supplier invoice number, batches and dates.", "If no more goods will arrive, use Close remainder on a partially received order and record the reason."], tips: ["The system highlights the lowest comparison value but never selects a supplier automatically.", "RFQs and requisitions do not move stock or post accounting.", "Receipt updates inventory, weighted cost, payable, general ledger and audit together in one database transaction."] },
  },
  {
