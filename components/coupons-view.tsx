...
export function CouponsView({ canManage = false }: { canManage?: boolean }) {
  const { money, dateTime, profile } = useBusiness();
  const [discountType, setDiscountType] = useState<"PERCENT" | "FIXED">("PERCENT");
  const amountStep = 10 ** -currencyFractionDigits(profile.currency);
  ...
  const redeemed = coupons.reduce((sum, coupon) => sum + Number(coupon.usageCount || 0), 0);
  return <div className="page page-enter">
    ...
    <section className="panel resource-panel">{loading ? <LoadingPanel /> : coupons.length ? <div className="coupon-list">{coupons.map((coupon) => { const expired = new Date(coupon.expiresAt) <= now; const scheduled = new Date(coupon.startsAt) > now; return <article className="coupon-ticket" key={coupon._id}><div className="coupon-cut" aria-hidden="true" /><header><span>{coupon.name}</span><StatusPill value={expired ? "EXPIRED" : scheduled ? "SCHEDULED" : coupon.active ? "ACTIVE" : "PAUSED"} /></header><div className="coupon-value"><BadgePercent /><strong>{coupon.type === "PERCENT" ? `${coupon.value}%` : money.format(coupon.value)}</strong><span>OFF</span></div><button className="coupon-code" onClick={() => navigator.clipboard.writeText(coupon.code)} title="Copy coupon code"><b>{coupon.code}</b><Copy size={14} /></button><dl><div><dt>Minimum</dt><dd>{money.format(coupon.minSpend)}</dd></div><div><dt>Used</dt><dd>{coupon.usageCount || 0}{coupon.usageLimit ? ` / ${coupon.usageLimit}` : " / ∞"}</dd></div><div><dt>Per member</dt><dd>{coupon.perMemberLimit || "Unlimited"}</dd></div><div><dt>Expires</dt><dd>{dateTime.format(new Date(coupon.expiresAt))}</dd></div></dl>{canManage ? <footer>{scheduled && !expired ? <button className="button button-primary" onClick={() => startNow(coupon)}>Start now</button> : null}<button className="button button-secondary" onClick={() => toggle(coupon)}>{coupon.active ? "Pause" : "Activate"}</button><button className="button button-quiet danger" onClick={() => archive(coupon)}><Archive size={14} />Archive</button></footer> : null}</article>; })}</div> : <EmptyState title="No coupons yet" detail="Generate a controlled percentage or fixed-value offer for the counter." action={canManage ? <AddButton onClick={() => setOpen(true)}>Generate coupon</AddButton> : undefined} />}</section>
    <Modal open={open} onClose={() => setOpen(false)} title="Generate coupon" kicker="CONTROLLED OFFER"><form className="modal-form" onSubmit={create}><div className="coupon-code-builder"><label className="field"><span>Coupon code</span><input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} pattern="[A-Z0-9_-]{3,32}" required autoFocus /></label><button type="button" className="button button-secondary" onClick={() => setCode(randomCode())}><Sparkles size={15} />Generate</button></div><label className="field"><span>Offer name</span><input name="name" placeholder="Weekend ceremonial matcha" required /></label><div className="form-grid two"><label className="field"><span>Discount type</span><select name="type" value={discountType} onChange={event => setDiscountType(event.target.value as "PERCENT" | "FIXED")}><option value="PERCENT">Percentage</option><option value="FIXED">Fixed amount</option></select></label><label className="field"><span>Discount value</span><input name="value" type="number" min={discountType === "PERCENT" ? 0.01 : amountStep} max={discountType === "PERCENT" ? 100 : 1000000} step={discountType === "PERCENT" ? 0.01 : amountStep} required /></label></div><div className="form-grid three"><label className="field"><span>Minimum spend</span><input name="minSpend" type="number" min="0" step={amountStep} defaultValue="0" /></label><label className="field"><span>Total uses · 0 unlimited</span><input name="usageLimit" type="number" min="0" step="1" defaultValue="0" /></label><label className="field"><span>Per member · 0 unlimited</span><input name="perMemberLimit" type="number" min="0" step="1" defaultValue="0" /></label></div><div className="form-grid two"><label className="field"><span>Starts</span><input name="startsAt" type="datetime-local" defaultValue={localDateTime(now)} required /></label><label className="field"><span>Expires</span><input name="expiresAt" type="datetime-local" defaultValue={localDateTime(new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000))} required /></label></div><p className="form-hint"><CalendarClock size={14} /> Enter dates in this device’s time zone ({Intl.DateTimeFormat().resolvedOptions().timeZone}). Saved expiry is shown in the workspace time zone. Dates and limits are checked at checkout.</p><footer><button type="button" className="button button-secondary" onClick={() => setOpen(false)}>Cancel</button><button className="button button-primary" disabled={busy}><Plus size={16} />{busy ? "Generating…" : "Generate coupon"}</button></footer></form></Modal>
  </div>;
}
