...
export function NfcControl({ onRead, onGenericRead, stopAfterGeneric = false, writeToken, disabled = false, autoStart = false, alwaysOn = false }: { onRead?: (code: string) => void | Promise<void>; onGenericRead?: (code: string) => void | Promise<void>; stopAfterGeneric?: boolean; writeToken?: string; disabled?: boolean; autoStart?: boolean; alwaysOn?: boolean }) {
  ...
  return <section className={`nfc-control no-print ${busy && !pageHidden ? "nfc-listening" : ""}`} data-nfc-auto-start={autoStart ? "true" : "false"} data-nfc-always-on={alwaysOn ? "true" : "false"}><div className="nfc-control-heading"><span className="nfc-signal" aria-hidden="true"><Nfc size={18} /></span><div><strong>{writeToken ? "Write NFC member card" : "NFC reader"}</strong><small>{busy ? pageHidden ? "RESUMES WHEN THIS PAGE IS VISIBLE" : "LISTENING LIVE · HOLD A CARD NEAR THE PHONE" : autoStart ? "AUTO-READY · SEPARATE FROM BARCODE SCANNER" : "SEPARATE CARD READER"}</small></div></div><button type="button" className="button button-secondary" disabled={!supported || disabled || (alwaysOn && busy)} onClick={() => void begin()} aria-label={busy ? alwaysOn ? "NFC reader listening" : "Stop NFC reader" : writeToken ? "Write member card" : "Start NFC reader"}><Nfc size={18} />{busy ? "NFC listening" : writeToken ? "Write member card" : autoStart ? "Start NFC (if needed)" : "Start NFC reader"}</button><p aria-live="polite">{message || defaultMessage}</p></section>;
}
