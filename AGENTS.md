...
- On 2026-09-19, `main` reached `4340da5` with controlled annual budget revisions, posted-ledger variance, cost-centre/project management accounting, exact-match automatic percentage-allocation rules, customer-default/invoice-override snapshots, and product-default POS classification with exact refund inheritance. The follow-up Atlas M0 hotfix restores the verified v8 index baseline and protects new uniqueness constraints with transactional business-key locks.
- `npm run typecheck`, `npm test` (195 tests), and the Webpack production build (67 generated pages) passed after the register-shift, exception-review, multi-location transfer, batch/expiry, freshness-forecast, inventory-disposal, MongoDB Atlas M0/Stable API compatibility, serverless cold-start performance, customer-account credit controls, quotation-to-invoice, quote-linked delivery-order, controlled CSV bank-reconciliation, transactional month-end-close, fixed-asset/depreciation/disposal, employee-expense/private-evidence, authenticated evidence-preview, multi-file expense-draft, versioned-budget/variance, cost-centre/project, deterministic percentage-allocation, invoice-dimension-default/override, product-level POS/refund classification, and business-key-lock upgrades. On this Windows host, the successful build used `KONKON_DISABLE_WEBPACK_CACHE=1` after Webpack's optional pack cache hit a filesystem write error.
