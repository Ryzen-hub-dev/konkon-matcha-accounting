...
- `npm run typecheck`, `npm test` (125 tests), and the Webpack production build (54 static pages) passed after the register-shift, exception-review, multi-location transfer, batch/expiry, freshness-forecast, and inventory-disposal upgrades. On this Windows host, the successful build used `KONKON_DISABLE_WEBPACK_CACHE=1` after Webpack's optional pack cache hit a filesystem write error.
