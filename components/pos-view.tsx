...
import {
  ...
  ReceiptText, Search, ShoppingBasket, Smartphone, TicketPercent, Trash2, UserRound,
} from "lucide-react";
...
export function PosView({ canManageTemplates = false, canManualDiscount = false }: { canManageTemplates?: boolean; canManualDiscount?: boolean }) {
  ...
  async function load(showLoading = true) {
    ...
    try {
      const [productData, memberData, templateData, paymentData] = await Promise.all([
        apiRequest<ProductRecord[]>("/api/products"), apiRequest<MemberRecord[]>("/api/members"), apiRequest<TemplateConfig>("/api/receipt-templates"), apiRequest<PaymentMethodRecord[]>("/api/payment-methods"),
      ]);
      setProducts(productData); setMembers(memberData); setTemplates(templateData.templates); setRegister(templateData.register); setPaymentMethods(paymentData);
      setPayment((current) => paymentData.some((method) => method.code === current) ? current : (paymentData.find((method) => method.code === "PAYNOW") || paymentData[0])?.code || "");
  ...
  function selectPayment(method: PaymentMethodRecord) { setPayment(method.code); setPaymentReference(""); if (method.kind === "CASH") setTenderedAmount(total); }
  ...
  async function checkout() {
    ...
    if (isCashPayment && tenderedAmount < total) return show("Enter enough cash to cover the amount due.", "error");
    if (selectedPayment.referenceRequired && !paymentReference.trim()) return show(`Enter the ${selectedPayment.name} reference before checkout.`, "error");
    setBusy(true);
    try {
      const result = await apiRequest<SaleReceipt>("/api/sales", { method: "POST", body: JSON.stringify({ memberId: memberId || null, paymentMethod: payment, paymentReference, tenderedAmount: isCashPayment ? tenderedAmount : total, templateId: selectedTemplateId, saleNote, manualDiscount, couponCode: appliedCouponCode, items: cart.map((line) => ({ productId: line._id, quantity: line.quantity })) }) });
  ...
  return <div className="page page-enter pos-page">
    <PageHeader eyebrow="COUNTER" title="Point of sale" description="Scan products, member cards and coupons through one trusted register." action={<div className="pos-page-actions"><Link className="button button-secondary" href="/receipts"><ReceiptText size={17} />Receipt history</Link>{canManageTemplates ? <button className="button button-secondary" onClick={() => setStudioOpen(true)}><Palette size={17} />Receipt templates</button> : null}</div>} />
    {notice ? <Notice {...notice} /> : null}
    <ScannerBridge contextLabel="Point of sale" purpose="POS" enabled={!loading} placeholder="Scan product · member card · coupon" onScan={handleScan} onFeedback={show} />
    {loading ? <LoadingPanel label="Opening the register…" /> : <div className="pos-layout"><section className="catalog-panel"><div className="catalog-toolbar"><label className="search-box"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tea, barcode, dōgu or SKU" /></label><span>{filtered.length} items</span></div><div className="category-tabs" role="tablist">{categories.map((name) => <button key={name} className={category === name ? "active" : ""} onClick={() => setCategory(name)}>{name}</button>)}</div>{filtered.length ? <div className="product-grid">{filtered.map((product) => <button className="product-card" key={product._id} onClick={() => add(product)} disabled={product.stock <= 0}><div className="product-top"><span>{product.category}</span><i className={product.stock <= product.reorderLevel ? "low" : ""}>{product.stock ? `${product.stock} left` : "Sold out"}</i></div><div className="product-glyph" aria-hidden="true"><span>{product.name.toLowerCase().includes("hojicha") ? "焙" : product.category === "Dōgu" ? "道" : "抹"}</span></div><strong>{product.name}</strong><small>{product.sku}{product.barcode ? ` · ${product.barcode}` : ""}</small><footer><b>{money.format(product.price)}</b><span><Plus size={16} /></span></footer></button>)}</div> : <EmptyState title="No tea found" detail="Try a different search or category." />}</section>
      <aside className="cart-panel"><header><div><span className="eyebrow light">CURRENT ORDER</span><h2>Tea tray <b>{cart.reduce((sum, line) => sum + line.quantity, 0)}</b></h2></div><ShoppingBasket /></header><label className="member-select"><UserRound size={17} /><select value={memberId} onChange={(event) => setMemberId(event.target.value)}><option value="">Walk-in guest</option>{members.map((member) => <option key={member._id} value={member._id}>{member.name} · {member.points} pts</option>)}</select><ChevronRight size={16} /></label><div className="cart-lines">{cart.length ? cart.map((line) => <div className="cart-line" key={line._id}><div><strong>{line.name}</strong><span>{money.format(line.price)} each</span></div><div className="qty-control"><button onClick={() => quantity(line._id, -1)}>{line.quantity === 1 ? <Trash2 size={14} /> : <Minus size={14} />}</button><b>{line.quantity}</b><button onClick={() => quantity(line._id, 1)} disabled={line.quantity >= line.stock}><Plus size={14} /></button></div><strong>{money.format(line.price * line.quantity)}</strong></div>) : <EmptyState title="The tray is empty" detail="Choose or scan a product to begin." />}</div>
        ...
        <div className="payment-methods"><span>PAYMENT</span><div>{paymentMethods.map((method) => { const Icon = method.kind === "CASH" ? Banknote : method.code.includes("CARD") ? CreditCard : Smartphone; return <button type="button" key={method._id} className={payment === method.code ? "active" : ""} onClick={() => selectPayment(method)} title={`Posts to ${method.accountCode} · ${method.accountName}`}><Icon size={17} />{method.name}</button>; })}</div></div>{isCashPayment ? <div className="cash-tender"><label><span>Cash received</span><input type="number" min={total} step="0.01" value={tenderedAmount || ""} onChange={(event) => setTenderedAmount(Math.max(0, Number(event.target.value)))} placeholder={total.toFixed(2)} /></label><div>{cashOptions.map((value) => <button type="button" key={value} onClick={() => setTenderedAmount(value)}>{money.format(value)}</button>)}</div><p><span>Change due</span><strong>{money.format(Math.max(0, Math.round((tenderedAmount - total + Number.EPSILON) * 100) / 100))}</strong></p></div> : <label className="register-reference"><span>{selectedPayment?.name || "Payment"} reference · {selectedPayment?.referenceRequired ? "required" : "optional"}</span><input value={paymentReference} maxLength={80} required={selectedPayment?.referenceRequired} onChange={(event) => setPaymentReference(event.target.value)} placeholder="Transaction or approval number" /></label>}<div className="register-paper"><label><span>Receipt template</span><select value={selectedTemplateId} onChange={(event) => setSelectedTemplateId(event.target.value)}>{templates.map((template) => <option key={template._id} value={template._id}>{template.name}{template.isDefault ? " · default" : ""}</option>)}</select></label><label><span>Order note · optional</span><input value={saleNote} maxLength={300} onChange={(event) => setSaleNote(event.target.value)} placeholder="Gift, collection or customer note" /></label></div><button className="checkout-button" disabled={!cart.length || !selectedTemplateId || !selectedPayment || busy || (isCashPayment && tenderedAmount < total) || Boolean(selectedPayment?.referenceRequired && !paymentReference.trim())} onClick={checkout}><span>{busy ? "Posting sale…" : "Complete sale"}</span><strong>{money.format(total)}</strong><ChevronRight size={20} /></button></aside></div>}
