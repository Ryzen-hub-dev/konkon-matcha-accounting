...
export function MemberCardsManager({ memberId, memberName, canWrite = false }: { memberId: string; memberName: string; canWrite?: boolean }) {
  ...
  async function bindExisting(code: string) {
    if (busy) return;
    setBusy(true); setError("");
    try {
      await apiRequest("/api/member-cards", { method: "POST", body: JSON.stringify({ action: "BIND", memberId, code, clientRequestId: crypto.randomUUID(), label: "Existing NFC card", tier: "MATCHA CLUB", accentColor: "#173f2a" }) });
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not bind this NFC card."); throw reason; }
  ...
  return <section className="member-cards-manager"><header className="no-print"><span className="eyebrow">MEMBER EXCLUSIVE CARDS</span><h2>QR & NFC cards</h2><p>Each issued card can be paused or replaced independently. A card identifies a member; it does not authorize payments.</p></header>
    ...
    {canWrite ? <section className="existing-nfc-bind no-print"><div><span className="eyebrow">USE WHAT YOU ALREADY HAVE</span><h3>Bind an existing NFC card</h3><p>Tap a readable hotel, transit or other NFC card on this phone. We record only a protected fingerprint; the card is not written or changed.</p></div><NfcControl onGenericRead={bindExisting} stopAfterGeneric disabled={busy} /><small>Some payment and hotel cards intentionally block browser NFC access. If the phone reports no readable identity, use a QR card or a supported USB/NFC reader.</small></section> : null}
    <div className="member-issued-cards no-print">{cards.length ? cards.map(card => <article key={card._id}><div><strong>{card.label}</strong><small>{card.kind === "BOUND" ? `Existing NFC · ${card.bindingSource === "NFC_SERIAL" ? "hardware serial" : "NDEF fingerprint"}` : "Kōn-Kōn issued card"} · •••• {card.last4} · {card.status}</small></div>{canWrite ? <div className="card-actions">{card.kind !== "BOUND" && card.status === "ACTIVE" ? <button className="button button-secondary" onClick={() => void reveal(card)} disabled={busy}>QR / NFC / print</button> : null}{["ACTIVE", "SUSPENDED"].includes(card.status) ? <><button className="button button-secondary" onClick={() => setEditing(card)}>Edit style</button><button className="button button-secondary" disabled={busy} onClick={() => void change(card, card.status === "ACTIVE" ? "SUSPENDED" : "ACTIVE")}>{card.status === "ACTIVE" ? "Suspend" : "Reactivate"}</button><button className="button button-secondary" disabled={busy} onClick={() => void change(card, "VOID")}>Void</button></> : null}{card.status !== "DELETED" ? <button className="button button-secondary" disabled={busy} onClick={() => void change(card, "DELETED")}>Delete</button> : null}</div> : null}</article>) : <p>No QR / NFC cards have been issued yet.</p>}</div>
    {selected && credential ? <section className="member-credential-stage"><article className="member-exclusive-card" data-print-qr style={{ borderColor: selected.accentColor, color: selected.accentColor }}><header>KŌN-KŌN MATCHĀ</header><small>{selected.tier}</small><h2>{memberName}</h2><QrImage value={credential} label={`Member card for ${memberName}`} width={160} /><p>{selected.label} · •••• {selected.last4}</p></article><div className="no-print"><button className="button button-primary" onClick={() => void printReceipt().catch(error => setError(error.message))}>Print member card</button><NfcControl writeToken={credential} /><details><summary>Write this card using another phone</summary><p>Scan this private link with the phone camera, then tap “Write member card”.</p><QrImage value={writerUrl} label="Open the NFC card writer on a phone" width={180} /><a className="button button-secondary" href={writerUrl} target="_blank" rel="noreferrer">Open NFC writer</a></details></div></section> : null}
  </section>;
}
