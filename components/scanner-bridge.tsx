...
type ScannerSession = { _id: string; label: string; expiresAt: string; connectedAt?: string; lastUsedAt?: string };
...
export function ScannerBridge({
  ...
  const loadSessions = useCallback(async (preferredId?: string) => {
    try {
      ...
      setSelectedId((current) => {
        ...
        return result.sessions.some((session) => session._id === candidate) ? candidate : result.sessions[0]?._id || "";
      });
    } catch (reason) {
  ...
  useEffect(() => {
    ...
    const listen = async () => {
      ...
      while (!stopped) {
        ...
        try {
          const events = await apiRequest<ScanEvent[]>(`/api/mobile-scans?sessionId=${selectedId}&consumerId=${encodeURIComponent(consumerIdRef.current)}&wait=1`, { signal: controller.signal });
          if (stopped) break;
          setBridgeState("LIVE");
  ...
  return <>
    <form className={`scanner-bridge scanner-bridge-${bridgeState.toLowerCase()}`} onSubmit={submit}>
      ...
      <button type="button" className="scanner-connect" onClick={() => { setOpen(true); void loadSessions(); }}><Smartphone />{selected ? "Phone linked" : "Link phone"}</button>
    ...
    <Modal open={open} onClose={() => setOpen(false)} title="24-hour phone scanner" kicker="RESTRICTED PASS">
      <div className="scanner-link-panel">
        <div className="scanner-link-intro"><Link2 /><div><strong>Automatic, one-purpose access</strong><p>Open the pass on a phone and this page selects the newest active device automatically. The phone can only send scanned values; it cannot read products, customers, prices or reports.</p></div></div>
        ...
        {issuedUrl ? <div className="issued-scanner-link"><span>COPY ONCE · OPEN ON THE PHONE</span><code>{issuedUrl}</code><button type="button" className="button button-secondary" onClick={() => navigator.clipboard.writeText(issuedUrl)}><Copy />Copy secure link</button></div> : null}
        <div className="scanner-session-list">{sessions.length ? sessions.map((session) => <article key={session._id} className={selectedId === session._id ? "listening" : ""}><div><strong>{session.label}</strong><span>{session.connectedAt ? "Phone connected" : "Waiting for phone"} · expires {new Intl.DateTimeFormat("en-SG", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(session.expiresAt))}</span></div><button type="button" className="button button-secondary" onClick={() => setSelectedId(session._id)} disabled={selectedId === session._id}>{selectedId === session._id ? "Listening" : "Use here"}</button><button type="button" className="icon-button danger" title="Revoke scanner pass" onClick={() => void revokeSession(session)}><Unplug /></button></article>) : <p className="scanner-empty">No active phone passes. Issue one to connect automatically.</p>}</div>
      </div>
    </Modal>
  </>;
}
