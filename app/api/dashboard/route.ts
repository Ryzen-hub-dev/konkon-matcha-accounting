...
export async function GET() {
  ...
  try {
    ...
    const localSaleDate = () => ({ $dateToString: { format: "%Y-%m-%d", date: "$createdAt", timezone: settings.timeZone } });
    const matchPeriod = (from: string) => ({
      ...
      $expr: { $and: [
        { $gte: [localSaleDate(), from] },
        { $lte: [localSaleDate(), period.today] },
      ] },
    });
    const periodStart = period.month < period.days[0] ? period.month : period.days[0];
    const [salesOverview, memberCount, lowStockCount, recentSales] = await Promise.all([
      db.collection("sales").aggregate([
        { $match: matchPeriod(periodStart) },
        { $facet: {
          today: [
            { $match: { $expr: { $eq: [localSaleDate(), period.today] } } },
          ...
          month: [
            { $match: { $expr: { $gte: [localSaleDate(), period.month] } } },
            { $group: { _id: null, revenue: { $sum: { $subtract: ["$total", { $ifNull: ["$refundedAmount", 0] }] } }, transactions: { $sum: 1 } } },
          ],
          dailySales: [
            { $group: { _id: localSaleDate(), total: { $sum: { $subtract: ["$total", { $ifNull: ["$refundedAmount", 0] }] } } } },
            { $sort: { _id: 1 } },
          ],
          topProducts: [
            { $match: { $expr: { $gte: [localSaleDate(), period.month] } } },
    ...
    const todayAgg = salesOverview?.today?.[0];
    const monthAgg = salesOverview?.month?.[0];
    const dailySales = salesOverview?.dailySales || [];
    ...
    return ok(serialise({
      ...
      lowStockCount,
      recentSales,
      dailySales: period.days.map(day => ({ _id: day, total: dailySales.find((point: { _id: string; total: number }) => point._id === day)?.total || 0 })),
