...
export async function POST(request: Request) {
  ...
  const input = receiptTemplateInputSchema.safeParse(body.value);
  if (!input.success) return fail("Check the receipt template details.", 422, input.error.flatten().fieldErrors);
  ...
  try {
    ...
    try {
      await mongoSession.withTransaction(async () => {
        if (document.isDefault) {
          await db.collection("receiptTemplates").updateMany({}, { $set: { isDefault: false } }, { session: mongoSession });
        }
        const result = await db.collection("receiptTemplates").insertOne(document, { session: mongoSession });
        insertedId = result.insertedId;
        await writeAudit(db, auth.session, "receipt-template.create", "receipt-template", insertedId.toHexString(), { name: document.name }, mongoSession);
      });
    } finally {
    ...
    if (typeof error === "object" && error && "code" in error && String(error.code) === "11000") {
      return fail("A receipt template with this name already exists.", 409);
    }
    return publicError(error);
  }
}
...
export async function PATCH(request: Request) {
  ...
  if (!input.success || !ObjectId.isValid(input.data?.id || "")) {
    return fail("Check the receipt template details.", 422, input.success ? undefined : input.error.flatten().fieldErrors);
  }

  try {
    const db = await getDb();
    const _id = new ObjectId(input.data.id);
    const current = await db.collection("receiptTemplates").findOne({ _id, active: { $ne: false } });
    if (!current) return fail("This receipt template no longer exists.", 404);
    const { id: _ignored, ...template } = input.data;
    if (current.isDefault && !template.isDefault) template.isDefault = true;
    const update = { ...template, nameNormalized: template.name.toLocaleLowerCase("en-SG"), updatedAt: new Date() };
    const client = await getMongoClient();
    const mongoSession = client.startSession();
    let saved = current;
    try {
      await mongoSession.withTransaction(async () => {
        if (update.isDefault) {
          await db.collection("receiptTemplates").updateMany({ _id: { $ne: _id } }, { $set: { isDefault: false } }, { session: mongoSession });
        }
        const result = await db.collection("receiptTemplates").findOneAndUpdate(
          { _id },
          { $set: update },
          { returnDocument: "after", session: mongoSession },
        );
        if (!result) throw new Error("Receipt template changed while it was being saved.");
        saved = result;
        await writeAudit(db, auth.session, "receipt-template.update", "receipt-template", input.data.id, { name: update.name }, mongoSession);
      });
    } finally {
      await mongoSession.endSession();
    }
