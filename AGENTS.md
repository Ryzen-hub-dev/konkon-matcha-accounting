...
- `npm run typecheck`, `npm test` (161 tests), and the Webpack production build (60 generated pages) passed after the register-shift, exception-review, multi-location transfer, batch/expiry, freshness-forecast, inventory-disposal, MongoDB Atlas M0/Stable API compatibility, serverless cold-start performance, customer-account credit controls, quotation-to-invoice, quote-linked delivery-order, controlled CSV bank-reconciliation, transactional month-end-close, and fixed-asset/depreciation/disposal upgrades. On this Windows host, the successful build used `KONKON_DISABLE_WEBPACK_CACHE=1` after Webpack's optional pack cache hit a filesystem write error.
