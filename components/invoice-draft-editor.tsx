...
export type EditableInvoice = Customer & {
  ...
  templateId?: string; templateName?: string; businessSnapshot?: { currency?: string; locale?: string; taxName?: string };
};
...
export function InvoiceDraftEditor({ source, mode, templates, onClose, onSaved }: {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = retry.current || JSON.stringify({ ...customer, items, dueDate, templateId, ...(editing ? { action: "EDIT_DRAFT", id: source!._id, expectedUpdatedAt: source!.updatedAt } : { clientRequestId: requestId.current }) });
    try {
