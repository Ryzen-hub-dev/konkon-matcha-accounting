...
import { CheckCircle2, ClipboardList, Clock3, CopyCheck, Eye, FilePlus2, PackageCheck, Pencil, Plus, Printer, Search, Send, Trash2, XCircle } from "lucide-react";
...
type Quotation = {
  ...
  deliveryOrderId?: string; deliveryOrderNo?: string;
  ...
  return <div className="page page-enter quotation-page">
    ...
    <section className="panel resource-panel quotation-register">
      ...
      {loading ? <LoadingPanel label="Preparing quotations…" /> : visible.length ? <div className="data-list quotation-list"><div className="data-list-head"><span>Quotation</span><span>Customer</span><span>Valid until</span><span>Value</span><span>Status</span><span>Actions</span></div>{visible.map(quotation => <div className="data-row" key={quotation._id}>
        ...
        <div className="row-actions"><button className="button button-quiet" onClick={() => setViewing(quotation)}><Eye size={14} />View</button>{canWrite && quotation.status === "DRAFT" ? <><button className="button button-quiet" onClick={() => setEditor(quotation)}><Pencil size={14} />Edit</button><button className="button button-secondary" onClick={() => beginAction(quotation, "MARK_SENT")}><Send size={14} />Sent</button></> : null}{canWrite && quotation.status === "SENT" && quotation.effectiveStatus !== "EXPIRED" ? <button className="button button-primary" onClick={() => beginAction(quotation, "ACCEPT")}><CheckCircle2 size={14} />Accept</button> : null}{canWrite && quotation.status === "SENT" ? <button className="button button-quiet" onClick={() => beginAction(quotation, "REJECT")}><XCircle size={14} />Reject</button> : null}{canWrite && quotation.status === "ACCEPTED" ? <button className="button button-primary" onClick={() => beginAction(quotation, "CONVERT")}><FilePlus2 size={14} />Invoice</button> : null}{canWrite && ["DRAFT", "SENT", "ACCEPTED"].includes(quotation.status) ? <button className="button button-quiet" onClick={() => beginAction(quotation, "VOID")}>Void</button> : null}{quotation.deliveryOrderId ? <Link className="button button-secondary" href="/delivery-orders"><PackageCheck size={14} />{quotation.deliveryOrderNo}</Link> : null}{quotation.convertedInvoiceId ? <Link className="button button-secondary" href={`/invoices/${quotation.convertedInvoiceId}`}>{quotation.convertedInvoiceNo}</Link> : null}</div>
