...
const rolePermissions: Record<UserRole, ReadonlySet<Permission>> = {
  ...
  ADMIN: new Set(
    PERMISSIONS.filter((permission) => permission !== "owner.control"),
  ),
  MANAGER: new Set([
    "dashboard.read",
    "pos.sell",
...
> = {
  OWNER: {
    label: "Owner",
    summary:
      "Company authority, security controls and every operational permission.",
  },
  ADMIN: {
    label: "Admin",
    summary:
      "Runs the workspace and staff, but cannot transfer ownership or close the company.",
  },
  MANAGER: {
    label: "Manager",
    summary:
      "Runs assigned counters, sales, stock, members, purchasing and reports.",
  },
  ACCOUNTANT: {
    label: "Accountant",
    summary:
      "Maintains books, invoices, payables and reports without operational control.",
  },
  CASHIER: {
    label: "Cashier",
    summary:
      "Sells, serves members and reads stock, receipts, coupons and payment methods.",
  },
};

export const ACCESS_AREAS: ReadonlyArray<{
  label: string;
  read: Permission;
  manage?: Permission;
  useLabel?: string;
  ...
  { label: "Payroll", read: "payroll.read", manage: "payroll.write" },
  { label: "Accounting", read: "accounting.read", manage: "accounting.write" },
  {
    label: "Budgets & variance",
    read: "budgets.read",
    manage: "budgets.write",
  },
  {
    label: "Cost centres & projects",
    read: "reports.read",
  ...
  { label: "Reports", read: "reports.read", manage: "reports.design" },
  {
...
export function accessLevel(
  role: UserRole,
  area: (typeof ACCESS_AREAS)[number],
) {
  if (area.manage && hasPermission(role, area.manage)) return "MANAGE" as const;
  if (hasPermission(role, area.read))
    return (area.useLabel || "VIEW") as "USE" | "VIEW";
