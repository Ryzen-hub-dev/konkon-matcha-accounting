...
type Card = { _id: string; label: string; tier: string; accentColor: string; status: "ACTIVE" | "SUSPENDED" | "VOID" | "DELETED"; last4: string };
export function MemberCardsManager({ memberId, memberName, canWrite = false }: { memberId: string; memberName: string; canWrite?: boolean }) {
  ...
  return <section className="member-cards-manager"><header className="no-print"><span className="eyebrow">MEMBER EXCLUSIVE CARDS</span><h2>QR & NFC cards</h2><p>Each issued card can be paused or replaced independently. A card identifies a member; it does not authorize payments.</p></header>
    ...
    <div className="member-issued-cards no-print">{cards.length ? cards.map(card => <article key={card._id}><div><strong>{card.label}</strong><small>{card.tier} · •••• {card.last4} · {card.status}</small></div>{canWrite ? <div className="card-actions">{card.status === "ACTIVE" ? <button className="button button-secondary" onClick={() => void reveal(card)} disabled={busy}>QR / NFC / print</button> : null}{["ACTIVE", "SUSPENDED"].includes(card.status) ? <><button className="button button-secondary" onClick={() => setEditing(card)}>Edit style</button><button className="button button-secondary" disabled={busy} onClick={() => void change(card, card.status === "ACTIVE" ? "SUSPENDED" : "ACTIVE")}>{card.status === "ACTIVE" ? "Suspend" : "Reactivate"}</button><button className="button button-secondary" disabled={busy} onClick={() => void change(card, "VOID")}>Void</button></> : null}{card.status !== "DELETED" ? <button className="button button-secondary" disabled={busy} onClick={() => void change(card, "DELETED")}>Delete</button> : null}</div> : null}</article>) : <p>No QR / NFC cards have been issued yet.</p>}</div>
